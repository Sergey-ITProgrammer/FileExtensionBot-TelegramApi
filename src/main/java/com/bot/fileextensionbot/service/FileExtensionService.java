package com.bot.fileextensionbot.service;
...
public class FileExtensionService {
    ...
    public String getFileExtension(byte[] arrayOfFileBytes) {
        return getUnknownFileExtension(arrayOfFileBytes, getListOfSignatures());
    }

    private String getUnknownFileExtension(byte[] arrayOfFileBytes, Map<String, String> listOfFileSignature) {
        String extensions = "txt";

        StringBuilder fileSignature = new StringBuilder();

        for (byte j : arrayOfFileBytes) {
            fileSignature.append(Integer.toHexString(j & 0xff)).append(" ");
            if (fileSignature.length() > 35) break;
        }

        String signature = fileSignature.toString().toUpperCase().trim();

        for (String s : listOfFileSignature.keySet()) {
            for (int i = 3; i <= signature.length(); i += 2) {
                if (s.equals(signature.substring(0, i).trim())) {
                    extensions = listOfFileSignature.get(s);
                    break;
                }
            }
        }
    ...
    private Map<String, String> getListOfSignatures() {
        ...
        List<String> listOfFileSignature = Arrays.asList(list.split("\n"));

        Map<String, String> mapOfFileSignature = new LinkedHashMap<>();

        for (String s : listOfFileSignature) {
            ...
            mapOfFileSignature.put(signature.toString(), extension);
        }

        return mapOfFileSignature;
    }
}
